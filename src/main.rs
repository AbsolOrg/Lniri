...
fn default_config_path() -> Option<PathBuf> {
    // Prioritize ~/.config/Lniri/config.kdl, then ~/.config/lniri/config.kdl.
    // If neither exists, defaults to ~/.config/Lniri/config.kdl so load_or_create()
    // generates Lniri's configuration independently from standard Niri.
    let config_parent = env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| {
            ProjectDirs::from("", "", "niri")
                .and_then(|dirs| dirs.config_dir().parent().map(PathBuf::from))
        });

    if let Some(base) = config_parent {
        let upper_path = base.join("Lniri").join("config.kdl");
        if upper_path.exists() {
            return Some(upper_path);
        }

        let lower_path = base.join("lniri").join("config.kdl");
        if lower_path.exists() {
            return Some(lower_path);
        }

        // Default path to create when neither exists: ~/.config/Lniri/config.kdl
        return Some(upper_path);
    }

    let Some(dirs) = ProjectDirs::from("", "", "Lniri")
        .or_else(|| ProjectDirs::from("", "", "lniri"))
        .or_else(|| ProjectDirs::from("", "", "niri"))
    else {
